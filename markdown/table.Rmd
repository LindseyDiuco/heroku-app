...
load(glue("{code.path}time_data_{today}.rda"))

N <- nrow(patient.data)

sex.male <- sum(patient.data$sex == 1)
sex.female <- sum(patient.data$sex == 2)
sex.unknown <- N - sex.male - sex.female

median.age <- median(patient.data$age_estimateyears)
min.age <- min(patient.data$age_estimateyears)
max.age <- max(patient.data$age_estimateyears)
...
| Transferred to another facility|  `r de$transfer.outcome` (`r round(de$transfer.outcome/de$N.cases, 2)`)                      |
...
|                               |                                           |
| **Admitted to ICU/HDU?**     |                                  |
|  Yes                          |       `r de$N.icu` (`r de$N.icu.p`)                            |
|  No/Unknown                           |    `r de$N.no.icu` (`r de$N.no.icu.p`)                                  |
|                               |                                  |
...
|----------------------------|----------------------------------------|--------------------------------------|------------------------------------------|---------------------------------------|
| Length of hospital stay    | `r de$obs.mean.adm.outcome`            | `r de$obs.sd.adm.outcome`            | `r de$obs.median.adm.outcome`            | `r de$obs.iqr.adm.outcome`            | 
| Symptom onset to admission | `r  de$obs.mean.onset.adm`             | `r de$obs.sd.onset.adm`              | `r  de$obs.median.onset.adm`             | `r de$obs.iqr.onset.adm`              | 
|                            |                                        |                                      |                                          |                                       | 
| Admission to ICU entry     | `r round(mean(de$a.icu, na.rm=T), 1)`  | `r round(sd(de$a.icu, na.rm=T), 1)`  | `r round(median(de$a.icu, na.rm=T), 1)`  | `r round(IQR(de$a.icu, na.rm=T), 1)`  | 
| Duration of ICU            | `r round(mean(de$d.icu, na.rm=T), 1)`  | `r round(sd(de$d.icu, na.rm=T), 1)`  | `r round(median(de$d.icu, na.rm=T), 1)`  | `r round(IQR(de$d.icu, na.rm=T), 1)`  | 
|                            |                                        |                                      |                                          |                                       | 
| Admission to IMV           | `r round(mean(de$a.imv, na.rm=T), 1)`  | `r round(sd(de$a.imv, na.rm=T), 1)`  | `r round(median(de$a.imv, na.rm=T), 1)`  | `r round(IQR(de$a.imv, na.rm=T), 1)`  | 
| Duration of IMV            | `r round(mean(de$d.imv, na.rm=T), 1)`  | `r round(sd(de$d.imv, na.rm=T), 1)`  | `r round(median(de$d.imv, na.rm=T), 1)`  | `r round(IQR(de$d.imv, na.rm=T), 1)`  | 
|                            |                                        |                                      |                                          |                                       | 
| Admission to NIV           | `r round(mean(de$a.nimv, na.rm=T), 1)` | `r round(sd(de$a.nimv, na.rm=T), 1)` | `r round(median(de$a.nimv, na.rm=T), 1)` | `r round(IQR(de$a.nimv, na.rm=T), 1)` | 
| Duration of NIV            | `r round(mean(de$d.nimv, na.rm=T), 1)` | `r round(sd(de$d.nimv, na.rm=T), 1)` | `r round(median(de$d.nimv, na.rm=T), 1)` | `r round(IQR(de$a.nimv, na.rm=T), 1)` | 
